...
CREATE TABLE IF NOT EXISTS `broonge`.`bikes` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `bike_serial` VARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `broonge`.`gps_tracking` (
  ...
  `riding_data_id` INT NULL,
  `reg_date` DATETIME NULL,
  `complete_date` DATETIME NULL COMMENT 'riding_data_id 는 혹시 모른 중복체크 때문에 필요하다.',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
